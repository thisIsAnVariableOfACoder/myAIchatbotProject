...
async function pickNextQuestion(state) {
  ...
  state.lastQuestionTags = ['general'];
...
export const offlineApi = {
  ...
  async sendMessage({ conversation_id, message, user_id, request_more, profile }) {
    ...
    if (message) {
      ...
      // Local heuristic analysis only (no frontend LLM calls)
    ...
    // Local result generation in offline mode (no frontend LLM calls)
    ...
    if (enoughInfo && !request_more) {
      ...
      return {
        success: true,
        data: {
          bot_reply: 'Mình đã tổng hợp hồ sơ của bạn và tạo danh sách nghề nghiệp phù hợp dựa trên câu trả lời. Bạn có thể xem bảng gợi ý ở bên phải để so sánh nhé.',
